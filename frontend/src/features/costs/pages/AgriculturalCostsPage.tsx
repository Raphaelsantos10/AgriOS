...
import { CircleDollarSign, CloudUpload, Download, Plus, Trash2 } from "lucide-react";
...
import type { AgriculturalCost, AgriculturalCostCategory, AgriculturalCostDraft } from "../types/agriculturalCost";
...
export default function AgriculturalCostsPage() {
  ...
  const [localPending, setLocalPending] = useState(() => source === "supabase" ? agriculturalCostService.localCount() : 0);
  ...
  const loadCosts = useCallback(async () => {
    ...
    catch { setError("Não foi possível carregar os custos. Confirme a migração da Sprint 110 e a ligação ao Supabase."); }
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const created = await agriculturalCostService.create(source, draft);
      setCosts((current) => [created, ...current]);
      setDraft(emptyDraft);
    }
    catch { setError("O custo não foi guardado. Confirme a sessão, a migração e as políticas RLS."); }
    finally { setBusy(false); }
  }

  async function remove(id: string) {
    if (!window.confirm("Apagar este custo agrícola?")) return;
    try { await agriculturalCostService.remove(source, id); setCosts((current) => current.filter((cost) => cost.id !== id)); }
    catch { setError("Não foi possível apagar este custo."); }
  }
  async function importLocalCosts() {
    setImporting(true); setError("");
    try { await agriculturalCostService.migrateLocalToSupabase(); setLocalPending(0); await loadCosts(); }
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-emerald-700">Sprint 110 · Persistência operacional</p><h1 className="mt-1 text-3xl font-black text-slate-950">Custos agrícolas</h1><p className="mt-2 text-sm text-slate-600">Registos protegidos por conta quando a autenticação está ativa.</p></div>
      <div className="flex flex-wrap items-center gap-3"><OperationalDataSourceBadge source={source}/>{localPending > 0 ? <Button variant="secondary" loading={importing} onClick={() => void importLocalCosts()}><CloudUpload size={18}/> Importar {localPending} locais</Button> : null}<Button variant="secondary" onClick={() => downloadAgriculturalCostsCsv(filtered, new Date().toISOString())}><Download size={18}/> Exportar CSV</Button></div>
    </header>
    ...
    {loading ? <div className="rounded-2xl border border-slate-200 bg-white p-5 text-sm font-semibold text-slate-500">A carregar custos agrícolas…</div> : null}
    ...
    <Card className="p-5 sm:p-7">
      ...
      <form onSubmit={submit} className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="flex items-end justify-between gap-3 lg:col-span-1"><div><p className="text-xs text-slate-500">Total calculado</p><p className="text-xl font-black">{calculateCostTotal(draft).toFixed(2)} €</p></div><Button type="submit" loading={busy}><CircleDollarSign size={18}/> Registar</Button></div>
