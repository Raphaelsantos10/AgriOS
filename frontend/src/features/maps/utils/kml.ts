  ...
  return `
    <Placemark>
      ...
      <ExtendedData>
        ...
        <Data name="source"><value>FARPHA</value></Data>
      </ExtendedData>
      <Polygon>
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    ...
    <description>Exportado pela FARPHA em ${new Date().toISOString()}</description>
    <Style id="farpha-field">
      <LineStyle><color>ff287a3f</color><width>3</width></LineStyle>
      <PolyStyle><color>5539a85b</color></PolyStyle>
    </Style>
    ${placemarks.map((placemark) => placemark.replace("<Placemark>", '<Placemark><styleUrl>#farpha-field</styleUrl>')).join("\n")}
  </Document>
</kml>`;
}
...
export function exportFieldAsKML(field: Field, farm: Farm) {
  ...
  downloadText(buildKml([field], farm), `farpha-${filename}.kml`, "application/vnd.google-earth.kml+xml;charset=utf-8");
}

export function exportFieldsAsKML(fields: Field[], farm: Farm) {
  const filename = sanitizeFilename(farm.name) || "exploracao";
  downloadText(buildKml(fields, farm), `farpha-${filename}-talhoes.kml`, "application/vnd.google-earth.kml+xml;charset=utf-8");
}
