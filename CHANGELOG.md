...
- Capturas desktop mantêm o limite mínimo de 1.000 × 200 px.
- Captura mobile padrão de 390 × 844 validada corretamente.
- Captura cortada da matriz com 97 px continua bloqueada.
- Três testes de regressão adicionados às regras de documentação visual.

## Sprint 109.1 — Correção das capturas de documentação

- Captura da matriz alterada de um contentor de cabeçalho para a janela completa.
- Altura esperada passa de 97 px para a viewport documental de 960 px.
- Verificação mínima de dimensões permanece ativa para impedir imagens cortadas.
- Supabase, autenticação, módulos e dados preservados.

## Sprint 109 — Apresentação pública e documentação consolidada

- README atualizado com capturas reais da aplicação em execução.
- Manifesto de imagens com data, origem e política contra mockups não identificados.
- Verificador automático de existência, dimensão e referência das capturas.
- Fluxo Playwright reproduzível para página pública, login, cadastro, dashboard e diagnóstico.
- Guias separados para utilizadores, administradores, demonstração pública e equipa técnica.
- Estado real, requisitos externos e limitações preservados na apresentação.
- Playwright usado somente em desenvolvimento; nenhum navegador é incluído no pacote.
- Guias de instalação e publicação da Sprint 109 adicionados.

## Sprint 108 — Matriz de maturidade dos módulos

- Quarenta e quatro módulos classificados como reais, parciais, demonstrativos ou planeados.
- Todas as quarenta rotas funcionais cobertas pela matriz técnica.
- Persistência, fonte, evidência, limitação e próximo passo documentados por módulo.
- Diagnóstico FARPHA ampliado com filtros, pesquisa, contadores e exportação CSV.
- Dados demonstrativos de Intelligence, Precisão, Digital Twin e Automações declarados.
- Verificação automática impede novas rotas sem classificação de maturidade.
- Três testes automatizados protegem totais, pesquisa e exportação da matriz.
- Plano Mestre Pré-Publicação atualizado sem alterar dados ou Supabase.

## Sprint 107.9 — Identidade FARPHA e organização do repositório

- Histórico integral do projeto recuperado e protegido contra truncamentos.
- Identidade pública consolidada em FARPHA — Intelligence for Agriculture.
- Pacote frontend renomeado para `farpha-frontend`.
- Verificação automática de marca, manifesto, página pública e ativos oficiais.
- Páginas públicas V2/V3 e recursos antigos Vite/React removidos após auditoria de uso.
- `.gitattributes` adicionado para estabilizar LF/CRLF no Windows e no Git Bash.
- Índice documental, política de arquivo, governança da marca e guia de contribuição.
- Templates de pull request, erros e funcionalidades para organizar a continuidade.
- Instruções seguras para a futura renomeação do repositório AgriOS para FARPHA.
